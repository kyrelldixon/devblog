...
Let's use account registration as an example to explain this.
...
Afterwards, the frontend completely takes over and the user can register for an account.
...
that the correct page shows up with the data it needs based on a specific route.
...
* Server Side Rendering (aka Universal/Isomorpic Rendering) tools -> Next and Nuxt
...
When the user submits the register form, the email and password is sent to the backend. But what *is* the backend?
...
However, the frontend code only runs in the browser so it is considered seperate from the backend.
...
the data, saving it to the database, and sending a response to communicate whether the data is processed successfully or if there is an error.
...
During this process, the frontend has to seperately determine what to do while it waits.
